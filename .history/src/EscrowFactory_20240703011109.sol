...
contract EscrowFactory is IEscrowDeployer {
    ...
    ) external returns (address escrow)
    {
        ...
        if (s_arbiterFee >= price) revert Escrow__FeeExceedsPrice(price, arbiterFee);
        if (s_token.balanceOf(address(this)) < price) revert Escrow__MustDeployWithTokenBalance();
