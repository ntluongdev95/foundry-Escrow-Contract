...
contract EscrowFactory is IEscrowDeployer {
    ...
    function createNewEscrow(
        ...
        bytes32 salt = keccak256(abi.encodePacked(i_price, i_token, i_buyer, i_seller, i_arbiter, i_arbiterFee));
        address computedAddress =
            computeEscrowAddress(address(this), i_price, i_token, msg.sender, i_seller, i_arbiter, i_arbiterFee);
        ...
        emit EscrowCreated(address(escrow), i_buyer, i_seller, i_arbiter);
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            hex"ff",
                            deployer,
                            keccak256(abi.encodePacked(price, token, buyer, seller, arbiter, arbiterFee)),
                            initCodeHash
                        )
                    )
                )
