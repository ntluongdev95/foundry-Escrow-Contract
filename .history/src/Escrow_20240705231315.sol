...
contract Escrow is ReentrancyGuard {
    ...
    function resolveDispute(uint256 buyerAward) external onlyArbiter nonReentrant inState(State.Disputed) {
        ...
        if (buyerAward > 0) {
            i_token.safeTransfer(i_buyer, buyerAward);
        }
        if (i_arbiterFee > 0) {
            i_token.safeTransfer(i_arbiter, i_arbiterFee);
        }
        tokenBalance = i_token.balanceOf(address(this));
        if (tokenBalance > 0) {
            i_token.safeTransfer(i_seller, tokenBalance);
        }
    }
