...
contract EscrowFactory is IEscrowDeployer {
    ...
    function createNewEscrow(
        ...
        bytes32 salt = keccak256(abi.encodePacked(i_buyer, i_seller, i_arbiter, i_arbiterFee, i_price));
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            hex"ff",
                            deployer,
                            keccak256(abi.encodePacked(price, token,buyer,seller,arbiter,arbiterFee,PRICE)),
                            initCodeHash
                            )
                        )
                    )
