...
contract TestEscrow is Test{
    ...
    function testDeployEscrowFromFactory() public {
        vm.startPrank(BUYER);
        ERC20Mock(address()).mint(BUYER, PRICE);
        ERC20Mock(address()).approve(address(escrowFactory), PRICE);
        escrow = escrowFactory.newEscrow(PRICE, , SELLER, ARBITER, ARBITER_FEE, SALT1);
        vm.stopPrank();
        assertEq(escrow.getPrice(), PRICE);
        assertEq(address(escrow.getTokenContract()), address());
